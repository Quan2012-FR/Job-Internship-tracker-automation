...
DEFAULT_INPUT_CANDIDATES = (
    ...
    Path("sample_data") / "example_company_list.xlsx",
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        help="Path to source company workbook. If omitted, a workbook is found automatically.",
    )
    parser.add_argument(
        "--output",
...
def main() -> int:
    args = parse_args()
    try:
        source_workbook = resolve_input_path(args.input)
    except FileNotFoundError as exc:
        raise SystemExit(str(exc)) from exc

    cfg = build_config(
        source_workbook=str(source_workbook),
        output_workbook=str(resolve_project_path(args.output)),
        database_path=str(resolve_project_path(args.database)),
...
def resolve_input_path(
    ...
    if value:
        ...
        working_candidate = working_dir / supplied
        if working_candidate.is_file():
            return working_candidate.resolve()

        project_candidate = project_dir / supplied
    ...
    searched = ", ".join(str(project_dir / path) for path in DEFAULT_INPUT_CANDIDATES)
    raise FileNotFoundError(
        "No input workbook was found automatically. Add companies.xlsx to the project folder "
        f"or use --input. Searched: {searched}"
    )


def resolve_project_path(value: str, project_dir: Path = PROJECT_DIR) -> Path:
    supplied = Path(value).expanduser()
    if supplied.is_absolute():
        return supplied
    return (project_dir / supplied).resolve()
