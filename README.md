...
The tracker automatically looks for a workbook in this order:

1. `companies.xlsx`
2. `sample_data.xlsx`
3. `sample_data/example_company_list.xlsx`

You only need `--input` when you want to use a different workbook:

```powershell
python update_jobs.py --input "C:\Projects\Job Tracker\companies.xlsx"
```

The command also works from the outer repository folder and the `Job Tracker` workspace folder. Relative input, output, database, and log paths are resolved consistently against the project.
...
```text
engineering_job_dashboard_YYYYMMDD.xlsx
```
...
| Option                       | Purpose                                                         |
| ---------------------------- | --------------------------------------------------------------- |
| `--input`                    | Optional source workbook path. When omitted, one is found automatically. |
...
## No Workbook Was Found Automatically

Add a workbook named `companies.xlsx` to the project folder, keep the included `sample_data.xlsx`, or provide a custom workbook with `--input`.
...
```bash
python update_jobs.py --max-companies 5
```
