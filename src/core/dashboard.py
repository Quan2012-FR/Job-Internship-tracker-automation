...
DATED_DASHBOARD_RE = re.compile(r"^(?P<stem>.+)_(?P<date>\d{8})$")
LOGGER = logging.getLogger("engineering_job_tracker.dashboard")
...
def resolve_dashboard_output_path(output_path: Path, run_time: datetime | None = None) -> Path:
    run_time = run_time or datetime.now()
    base_stem = _dashboard_base_stem(output_path)
    return output_path.with_name(f"{base_stem}_{run_time:%Y%m%d}{output_path.suffix}")


def write_dashboard(conn: sqlite3.Connection, output_path: Path, stats: RunStats, priority_threshold: int) -> Path:
    resolved_output_path = resolve_dashboard_output_path(output_path)
    _sync_application_tracking(conn, resolved_output_path)
    ...
    _delete_previous_dashboards(resolved_output_path)
    return resolved_output_path


def _sync_application_tracking(conn: sqlite3.Connection, output_path: Path) -> None:
    tracking_source = _find_tracking_source(output_path)
    if tracking_source is None:
        return

    wb = load_workbook(tracking_source)
    try:
...
def _find_tracking_source(output_path: Path) -> Path | None:
    family_paths = _dashboard_family_paths(output_path)
    if output_path.exists():
        return output_path
    if not family_paths:
        return None
    return max(family_paths, key=lambda path: path.stat().st_mtime)


def _delete_previous_dashboards(output_path: Path) -> None:
    for path in _dashboard_family_paths(output_path):
        if path == output_path:
            continue
        try:
            ...
            LOGGER.warning(
                "Could not delete previous dashboard because it is in use: %s",
                path,
            )
        except OSError as exc:
            LOGGER.warning("Could not delete previous dashboard %s: %s", path, exc)


def _dashboard_family_paths(output_path: Path) -> list[Path]:
    ...
    for candidate in directory.iterdir():
        ...
        if candidate_stem == base_stem:
            ...
            continue
        match = DATED_DASHBOARD_RE.match(candidate_stem)
        if match and match.group("stem") == base_stem:
            family_paths.append(candidate)
    return family_paths


def _dashboard_base_stem(output_path: Path) -> str:
    match = DATED_DASHBOARD_RE.match(output_path.stem)
    return match.group("stem") if match else output_path.stem
